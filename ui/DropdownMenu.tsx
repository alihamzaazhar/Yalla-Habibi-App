  ...
  return (
    <>
      {open && (
        <View
          className={cn(
            "min-w-[8rem] w-full absolute flex gap-3 overflow-hidden rounded-md border border-border bg-background text-popover-foreground shadow-md mt-3 top-12 mx-auto justify-center z-50",
            className
          )}
        >
          {children}
...
const DropDownItem = ({ children, className }: DropDownItemProps) => {
  return (
    <View className={cn("p-2 rounded-md", className)}>
      {children}
    </View>
  );
};
